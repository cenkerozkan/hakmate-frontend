...
const smartSendMessage = async (requestBody) => {
  ...
  try {
    ...
    console.log(
      `Axios failed (${consecutiveAxiosFailures} consecutive failures), trying fetch...`
    );
...
const sendWithFetch = async (requestBody) => {
  const token =
    globalUserState.anonymousSession?.token ||
  ...
  try {
    const response = await fetch(
      "https://hakmate.cenkerozkan.com/api/rag/query",
      {
        method: "POST",
        headers: {
...
chatServiceAxios.interceptors.request.use(async (config) => {
  try {
    ...
    if (!globalUserState.anonymousSession) {
      try {
        const anonymousResponse = await fetch(
          "http://localhost:3001/api/auth/anonymous",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
          }
    ...
    const currentToken =
      globalUserState.anonymousSession?.token || "fallback_token";
...
export const ChatAPI = {
  ...
  createChatThread: async ({
    ...
    const currentUserId =
      userId || globalUserState.userId || (await ChatAPI.getCurrentUserId());
    ...
    const requestBody = {
      ...
      anonymous_user_id:
        globalUserState.anonymousSession?.sessionId || anonymousUserId,
    };
  ...
  getAllChatThreads: async (userId = null) => {
    ...
    return await chatServiceAxios.get(
      `/chat_service/get_all_chat_threads/${currentUserId}`
    );
  },
