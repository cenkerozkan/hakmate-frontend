...
export default function LawOffices() {
  ...
  // LawOffices.js içindeki searchOffices metodunu değiştirin
  const searchOffices = async () => {
    setLoading(true);
    try {
      console.log("Arama başlatılıyor. Konum bilgisi:", {
        hasLocation,
        latitude,
        longitude,
        radius,
      ...
      if (hasLocation) {
        console.log("Google API ile arama yapılıyor...");
        // nearby-google endpoint'i zaten Google API'yi dener, bulamazsa Supabase'e fallback yapar
        response = await LawOfficesAPI.getNearbyOfficesFromGoogle(
          latitude,
          longitude,
          radius
        );
        ...
        console.log("Konum yok, tüm bürolar getiriliyor...");
        response = await LawOfficesAPI.getAllOffices();
        console.log("Tüm bürolar yanıtı:", response.data);
      }

      let offices = response?.data || [];
      console.log(`${offices.length} adet büro bulundu.`);
      ...
      showError(
        "Hukuk büroları aranırken bir hata oluştu. Lütfen daha sonra tekrar deneyin."
      );
    } finally {
      setLoading(false);
    }
              ...
              >
                <Box
                  sx={{
                    ...
                    borderRadius: "50%",
                    background:
                      "linear-gradient(135deg, #8B1538 0%, #A91850 100%)",
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    mx: "auto",
